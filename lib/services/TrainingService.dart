...
class TrainingService {
  ...
  /// Mark a video as completed/watched by the current user
  static Future<void> markVideoAsWatched(String videoId) async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) throw Exception('User must be authenticated');
  ...
  static Stream<List<Map<String, dynamic>>> getUserTrainingProgress() {
    try {
      final user = FirebaseAuth.instance.currentUser;
  ...
  /// Get progress stats for all videos
  static Future<Map<String, dynamic>> getProgressStats() async {
    try {
      ...
      final totalWatched = progressSnapshot.docs.length;

      // Get total videos
      final allVideosSnapshot =
          await FirebaseFirestore.instance.collection('training_videos').get();
      final totalVideos = allVideosSnapshot.docs.length;

      return {
        'total_watched': totalWatched,
        'total_videos': totalVideos,
        'progress_percentage': totalVideos > 0
            ? ((totalWatched / totalVideos) * 100).toStringAsFixed(1)
            : '0.0',
        'watched_videos': progressSnapshot.docs
            .map((doc) => doc.id)
            .toList(),
      };
    } catch (e) {
  ...
  /// Get training progress by level
  static Future<Map<String, int>> getProgressByLevel(String level) async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) throw Exception('User must be authenticated');

      // Get videos with this level
      ...
      final completedCount = progressSnapshot.docs
          .where((doc) => videoIds.contains(doc.id))
          .length;
  ...
  /// Check if a video is watched by current user
  static Future<bool> isVideoWatched(String videoId) async {
    try {
      ...
      return doc.exists && (doc.data()?['is_watched'] ?? false) as bool;
    } catch (e) {
      print('Error checking if video is watched: $e');
      return false;
    }
  }
}
